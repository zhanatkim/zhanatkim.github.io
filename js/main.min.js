...
var _tariffCards = require("./modules/tariff-cards");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('load', function () {
    (0, _tariffCards.renderMembership)();
    (0, _initTabs.initTabs)();
    (0, _videoPlayer.videoPlay)();
    (0, _faq.setCurrentChapter)();
...
},{"./modules/accordions/init-accordion":3,"./modules/faq":4,"./modules/form-validate/form":6,"./modules/tabs/init-tabs":12,"./modules/tariff-cards":14,"./modules/video-player":15,"./utils/ios-vh-fix":17}],2:[function(require,module,exports){
"use strict";
...
exports.setCurrentChapter = void 0;
var buttonsContainer = document.querySelector('.faq__controls');
var chaptersContainer = document.querySelector('.faq__content');
function showChapter(evt) {
  ...
  var chaptersItems = Array.from(chaptersContainer.children);
  var currentChapter = chaptersItems.find(function (el) {
    return el.dataset.id === evt.target.dataset.id;
  });
  if (!currentChapter) {
    return;
  }
  for (var _i = 0; _i < chaptersContainer.children.length; _i++) {
    chaptersContainer.children[_i].classList.remove('is-open');
  }
  currentChapter.classList.add('is-open');
}
var setCurrentChapter = function setCurrentChapter() {
...
exports.renderMembership = void 0;
var tariff = [{
  id: 1,
  title: 'С тренером',
  feature: '12 занятий',
...
var container = document.querySelector('.tabs__content');
...
var createCard = function createCard(_ref) {
  ...
  cardTemplateClone.querySelector('.tariff__title').textContent = "".concat(title);
  cardTemplateClone.querySelector('.tariff__feature').textContent = "".concat(feature);
  cardTemplateClone.querySelector('.tariff__price').textContent = "".concat(price);
  cardTemplateClone.querySelector('.tariff__bg').textContent = "".concat(price);
...
var renderMembership = function renderMembership() {
  container.querySelectorAll('.tabs__element').forEach(function (el) {
    return el.remove();
  });
  createCardsList(tariff);
  var tariffHalfYear = [].concat(tariff).map(function (item) {
    item.price = item.price * 6 * 0.9;
    return item;
  });
  createCardsList(tariffHalfYear);
  var tariffFullYear = [].concat(tariff).map(function (item) {
    item.price = item.price * 12 * 0.8;
    return item;
  });
  createCardsList(tariffFullYear);
};
exports.renderMembership = renderMembership;
...
var video = videoContainer.querySelector('.video__player');
var placeholder = videoContainer.querySelector('.video__placeholder');
